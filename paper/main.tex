...
\begin{enumerate}[leftmargin=*]
    \item \textbf{H1---superiority:} the mean paired gain $A_{\mathrm{full}}-A_{\mathrm{match}}$ must be at least $+0.02$ and its paired seed-level 95\% bootstrap confidence interval must exclude zero. Any headline superiority claim must also hold against the strongest trained non-JEPA baseline.
...
\end{tabular}
\end{table}
The frozen validation fails H1. The paired mean $-0.0115254237$ is negative and therefore cannot satisfy the preregistered $+0.02$ practical-effect gate, so the evidence does not support superiority over the capacity-matched supervised baseline.
...
\end{tabular}
\end{table}
Full minus \texttt{no\_planner} was $+0.0047457627$ with retained bootstrap 95\% CI $[0.0, 0.0142372881]$: the mean is below the preregistered $+0.01$ H2 gate and the interval does not exclude zero. Full minus \texttt{no\_target} was $-0.0067796610$ with interval $[-0.0135593220,0.0]$: it likewise fails the $+0.01$ H3 gate and the interval does not exclude zero. Neither mechanism attribution is supported. The shuffled-label control remained below the frozen $0.35$ ceiling, which establishes only that the declared validity stop rule did not trigger; it is not favorable evidence for H1--H3.
...
Seed-level integer outcomes make the five-seed limitation explicit. Over the fixed 295-row validation set, full-minus-\texttt{no\_planner} correct-count differences were $[0,0,+7,0,0]$, while full-minus-\texttt{no\_target} differences were $[0,0,-5,0,-5]$. Thus the planner aggregate is driven by a single seed and the target-path aggregate by two seeds. Because the externally reviewed retained runs are constant classifiers, these sparse count shifts are consistent with changes in the class selected under collapse rather than evidence of stable input-conditional planner or target-path benefit.
...
\]
The repository's \texttt{MANUSCRIPT\_PROVENANCE.md} and \texttt{REPRODUCE.md} contain the current provenance edges, exact commands, artifact identifiers, and frozen stop rules. The defensible reproducibility statement is that independent project-controlled reruns reproduce the aggregate scientific conclusion and verifier outputs; low-level probabilities and checkpoint bytes are not claimed bitwise identical across all runners. One independent external frozen-protocol rerun/review also reproduced the retained headline metrics and bounded collapse diagnosis; additional independent reports remain pending.
