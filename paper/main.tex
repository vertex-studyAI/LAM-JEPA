...
The full-controls validation uses seeds $\{1,2,3,4,5\}$, 20 epochs, batch size 32, learning rate $3\times10^{-4}$, one model step, all eligible train and validation rows, and CPU execution. The pinned pretrained comparator uses \path{microsoft/deberta-v3-xsmall} at immutable revision \path{14809e4f1fe1895fcba8b258271a940c6ca45ec4}. Means and sample standard deviations are reported across seeds; mechanism effects are paired by seed and summarized with retained bootstrap 95\% intervals.
...
\[
\begin{aligned}
\text{claim} &\rightarrow \text{table/figure} \rightarrow \text{processed metric} \\
             &\rightarrow \text{raw retained artifact} \rightarrow \text{frozen config/protocol} \\
             &\rightarrow \text{source commit}.
\end{aligned}
\]
