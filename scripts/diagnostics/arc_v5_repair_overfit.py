...
def quantizer_state(model, outputs) -> dict[str, object]:
...
def evaluate(model, tokens, numeric_x, labels) -> dict[str, object]:
    ...
    return {
        ...
        "quantizer_state": quantizer_state(model, outputs),
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Frozen train-only acceptance gate for the explicit ARC v5 quantizer repair.")
    ...
    repaired_pass = bool(summaries["repaired_v5"]["all_seeds_reach_overfit_threshold"])
    payload = {
        "artifact_type": "LAM-JEPA ARC v5 quantizer repair trainability gate",
        "scope": "training-only primary repair acceptance; no validation or test access",
        "repair_id": ARC_V5_REPAIR_ID,
        "repair_spec": arc_v5_repair_spec(),
        "subset": {"rows": 32, "per_class": 8, "ids": [example.item_id for example in subset]},
        "training": {"seeds": list(SEEDS), "steps": 300, "learning_rate": 3e-4, "overfit_threshold": OVERFIT_THRESHOLD},
        "records": records,
        "summaries": summaries,
        "repair_trainability_gate_pass": repaired_pass,
        "verdict": "PRIMARY_REPAIR_TRAINABILITY_GATE_PASSED" if repaired_pass else "PRIMARY_REPAIR_TRAINABILITY_GATE_FAILED",
        "claim_boundary": {
    ...
    print(json.dumps({"verdict": payload["verdict"], "summaries": summaries, "repair_spec": payload["repair_spec"]}, indent=2))

    if not repaired_pass:
        raise SystemExit("repaired ARC v5 quantizer failed the frozen two-seed overfit gate")
