...
def main() -> None:
    ...
    # Scientific question and externally visible claim gates cannot drift under a schema repair.
    for key in ("scientific_question", "claim_scope", "training_budget", "claim_gate"):
    ...
    require(negative.get("permutation_seed") == 20260807, "negative-control seed changed")
    require("same number of choices" in negative.get("permutation_rule", ""), "cardinality-safe permutation rule missing")
    require("0.35" in negative.get("failure_rule", ""), "negative-control stop threshold changed")
    ...
    require(v3.get("metrics", {}).get("practical_effect_threshold_absolute") == 0.02, "practical effect threshold changed")
