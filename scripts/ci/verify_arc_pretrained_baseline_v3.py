...
def load_helpers():
    ...
    spec = importlib.util.spec_from_file_location("arc_pretrained_verifier_helpers", helper_path)
...
def main() -> None:
    ...
    require(protocol.get("frozen_protocol_id") == PROTOCOL_ID, "result is not bound to protocol v3")
    ...
    require(protocol.get("pretrained_model_id") == MODEL_ID, "pretrained model id mismatch")
    require(protocol.get("pretrained_model_revision") == MODEL_REVISION, "declared pretrained revision mismatch")
    require(protocol.get("resolved_pretrained_revision") == MODEL_REVISION, "resolved pretrained revision mismatch")
    require(protocol.get("pretrained_model_license") == MODEL_LICENSE, "pretrained license mismatch")
    require(protocol.get("transformers_version") == TRANSFORMERS_VERSION, "runtime transformers version mismatch")
    require(protocol.get("transformers_version_pin") == TRANSFORMERS_VERSION, "transformers pin mismatch")
    require(protocol.get("sentencepiece_version_pin") == SENTENCEPIECE_VERSION, "sentencepiece pin mismatch")
    ...
    require(parameter_count > 10_000_000, "pretrained parameter count implausibly small")
    ...
    require(all(isinstance(step, int) and step >= 1 for step in declared_steps), "pretrained baseline must train")

    validation_n = int(protocol.get("validation_examples", 0))
    require(validation_n > 0, "validation set is empty")
    canonical_ids = None
    canonical_labels = None
    canonical_reversed_labels = None
    ...
    for index, (expected_seed, record) in enumerate(zip(seeds, records, strict=True)):
        ...
        if canonical_ids is None:
            ...
            require(labels == canonical_labels, f"seed {expected_seed}: labels changed")
        ...
        pre_rev_metrics, pre_rev_ids, pre_rev_labels = metrics_from_rows(pretrained.get("choice_reversal_predictions"), canonical_ids, f"seed {expected_seed}/pretrained-reversed")
        lam_rev_metrics, lam_rev_ids, lam_rev_labels = metrics_from_rows(lam.get("choice_reversal_predictions"), canonical_ids, f"seed {expected_seed}/lam-reversed")
        ...
        require(math.isclose(float(record.get("accuracy_delta_lam_minus_pretrained")), delta, rel_tol=1e-9, abs_tol=1e-9), f"seed {expected_seed}: paired delta mismatch")
