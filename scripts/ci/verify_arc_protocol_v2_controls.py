...
def verify_metrics(declared: object, recomputed: dict[str, float], label: str) -> None:
    require(isinstance(declared, dict), f"{label}: metrics missing")
    for key, expected in recomputed.items():
        actual = float(declared.get(key, float("nan")))
        require(math.isclose(actual, expected, rel_tol=1e-6, abs_tol=1e-6), f"{label}: {key} mismatch")
...
def paired_bootstrap_ci(deltas: Sequence[float], *, seed: int, samples: int = 10000) -> tuple[float, float]:
    ...
    for _ in range(samples):
        boot.append(float(statistics.fmean(deltas[rng.randrange(n)] for _ in range(n))))
    boot.sort()
    return boot[int(0.025 * (samples - 1))], boot[int(0.975 * (samples - 1))]


def main() -> None:
    parser = argparse.ArgumentParser(description="Independently verify real-ARC protocol-v2 controls.")
    ...
    require(frozen.get("status") == "FROZEN_BEFORE_CONFIRMATORY_TEST", "protocol is not frozen")
    ...
    require("never use confirmatory test labels" in str(frozen_negative.get("split", "")), "negative control test isolation missing")
    ...
    require(protocol.get("train_validation_overlap") == 0, "train/validation leakage")
    ...
    for key in ("train_digest", "validation_digest", "train_id_digest", "validation_id_digest", "original_training_label_digest", "permuted_training_label_digest"):
        value = protocol.get(key)
        require(isinstance(value, str) and len(value) == 64, f"invalid digest: {key}")
    ...
    require(model_steps >= 1, "planner must be exercised in full/no-target variants")
    validation_n = int(protocol.get("validation_examples", 0))
    require(validation_n > 0, "validation set empty")
    ...
    for variant in ("full", "no_planner", "no_target"):
        ...
        require(isinstance(block, dict), f"{variant}: block missing")
        ...
        for expected_seed, record in zip(seeds, records, strict=True):
            ...
            reversed_observed = record.get("observed_reversed_action_steps")
            require(isinstance(observed, list) and observed and all(step == expected_actions for step in observed), f"{variant}: forward action execution mismatch")
            require(isinstance(reversed_observed, list) and reversed_observed and all(step == expected_actions for step in reversed_observed), f"{variant}: reversed action execution mismatch")
            ...
            require(len(ids) == validation_n, f"{variant}: validation row count mismatch")
            require(ids == rev_ids, f"{variant}: choice reversal changed item identity/order")
            require(rev_labels == [3 - label for label in labels], f"{variant}: reversed labels are not exact remaps")
            if canonical_ids is None:
                canonical_ids, canonical_labels = ids, labels
            else:
                require(ids == canonical_ids and labels == canonical_labels, f"{variant}: paired validation rows changed")
    ...
    for offset, variant in enumerate(("no_planner", "no_target")):
        ...
        require(block.get("seed_level_full_minus_ablation") == deltas, f"{variant}: seed-level paired deltas mismatch")
        ...
        require(math.isclose(float(block.get("mean_full_minus_ablation")), mean_delta, rel_tol=1e-12, abs_tol=1e-12), f"{variant}: mean paired delta mismatch")
        ...
        require(math.isclose(float(block.get("paired_bootstrap_ci95_low")), ci_low, rel_tol=1e-12, abs_tol=1e-12), f"{variant}: bootstrap low mismatch")
        require(math.isclose(float(block.get("paired_bootstrap_ci95_high")), ci_high, rel_tol=1e-12, abs_tol=1e-12), f"{variant}: bootstrap high mismatch")
        expected_observed_criterion = mean_delta >= 0.01 and ci_low > 0.0
        require(block.get("mechanism_claim_gate_met") is expected_observed_criterion, f"{variant}: declared mechanism criterion mismatch")
    ...
    for expected_seed, record in zip(seeds, records, strict=True):
        ...
        recomputed, ids, labels = metrics_from_rows(record.get("predictions"))
        require(ids == canonical_ids and labels == canonical_labels, "negative control changed validation rows/labels")
        observed = record.get("observed_action_steps")
    ...
    report = {
        "verdict": "PROTOCOL_V2_ARC_CONTROLS_EXECUTION_VERIFIED_ONLY",
