...
**Manuscript status:** evidence-backed working draft; internal paper package not yet externally validated or submission-ready.  
...
7. a source-level audit that constrains the architecture and novelty wording to what was actually tested.
...
I-JEPA predicts representations of distinct target image blocks from a context block and updates its target encoder by exponential moving average [Assran et al., 2023]. This establishes representation-space prediction and EMA target encoders as prior art. The frozen LAM ARC path differs in a scientifically important way: its online and target encoders receive the same serialized ARC input, and the ARC loss aligns the quantized online latent to that same-input EMA target. The present manuscript therefore uses conservative `target-encoder alignment` wording and does not claim to instantiate the canonical I-JEPA context-to-distinct-target prediction task.

Planning with JEPA/world-model representations is also established prior art. FF-JEPA explicitly introduces a latent planner in a JEPA/world-model setting [Masip et al., 2026]. The current work therefore does not claim that adding a latent planner to a JEPA-labelled system is a novel idea.

### 2.2 Vector-quantized representations

Vector-quantized discrete latent learning predates this project, including VQ-VAE [van den Oord et al., 2017]. LAM-JEPA's quantizer and later trainability repair are treated here as implementation/mechanism details, not as evidence of a novel generic quantization method.

### 2.3 ARC and pretrained language models

ARC was introduced as a challenging multiple-choice science-question benchmark [Clark et al., 2018]. The bounded pretrained comparison in this project uses a pinned DeBERTa-v3-xsmall checkpoint from the DeBERTaV3 model family [He et al., 2021/2023]. We do not compare the locked ARC test to public leaderboard numbers, and the development comparator is not used to claim broad inferiority.

### 2.4 Reproducibility practice

Reproducibility programmes and checklists are established in machine learning [Pineau et al., 2021]. The contribution here is not the invention of reproducibility or preregistration; it is the use of frozen protocols, retained adverse results, claim ledgers, artifact lineage, and explicit stop rules to make this particular negative result auditable.

### 2.5 Originality classification

The accompanying `ORIGINALITY_AUDIT.md` classifies the present package conservatively as a **novel empirical observation / useful engineering and reproducibility contribution**, not a substantial mechanism or theoretical contribution. The exact component combination may be project-specific, but the frozen evidence does not show that the combination confers an advantage.

## 3. Problem Formulation and Hypotheses

Let `A_full` denote validation accuracy of the frozen full LAM-JEPA ARC configuration; `A_match` the gradient-active-parameter-matched supervised comparator; `A_np` the `no_planner` ablation; and `A_nt` the `no_target` ablation.
...
The deterministic shuffled-label control was required to remain below a frozen accuracy ceiling of `0.35`. It is a validity check, not evidence for H1–H3.
...
The reported ARC benchmark uses `_lam_arc_loss`, not the repository's larger generic training loss. For an ARC minibatch,
...
The supervised comparator uses the same `MultiViewEncoder`/projector family and a four-choice classifier, without the LAM target/quantizer/memory/planner machinery. The frozen gradient-active parameter counts are:
...
- CPU execution in the retained independent rerun workflow.

The exact processor model is not asserted in this draft; provenance is tied to retained workflow/artifact metadata rather than inferred hardware.
...
### 8.5 Reproducibility defect

A pre-fix seed-order defect was found: model initialization occurred before the requested seed was applied. PR #61 / SHA `b72a97a99769b278eb8ec75bc5eab62dc9599f29` repaired the reproducibility plumbing without changing the frozen scientific protocol. Failed/pre-fix evidence remains retained.
...
7. The exact CPU model/environment description still needs final publication metadata consolidation, although retained workflows identify source, commands, artifacts, and execution class.
8. Bibliographic novelty has been audited conservatively, but independent expert review is still absent. **[EXTERNAL VALIDATION REQUIRED]**
...
The paper package aims to trace each quantitative manuscript statement through:

`claim → table/figure → processed metric → raw retained artifact → frozen config/protocol → scientific source commit`.

Current core provenance anchors are the frozen scientific source SHA `760aa7f9a73a177d5ff4ba7eb470f7e68ace63cb`, retained full-controls artifacts, `CLAIM_LEDGER.md`, `REPRODUCE.md`, `EVIDENCE_AUDIT_20260813.md`, `paper/ARC_NEGATIVE_RESULT_TABLES.md`, and the reproducibility-wave evidence bundle.
...
3. Peter Clark, Isaac Cowhey, Oren Etzioni, Tushar Khot, Ashish Sabharwal, Carissa Schoenick, Oyvind Tafjord. **Think you have Solved Question Answering? Try ARC, the AI2 Reasoning Challenge.** arXiv:1803.05457, 2018.
4. Pengcheng He, Jianfeng Gao, Weizhu Chen. **DeBERTaV3: Improving DeBERTa using ELECTRA-Style Pre-Training with Gradient-Disentangled Embedding Sharing.** arXiv:2111.09543, 2021; ICLR 2023.
